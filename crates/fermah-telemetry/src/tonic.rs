...
impl Telemetry for TonicTelemetry {
    ...
    fn with_logs(mut self) -> Self {
        ...
        let logs = opentelemetry_otlp::new_pipeline()
            ...
            .with_log_config(
                opentelemetry_sdk::logs::config().with_resource(self.create_resource()),
            )
    ...
    fn with_service_name(mut self, name: String) -> Self {
        self.service_name = Some(name);
        self
    }

    fn with_env(mut self, env: String) -> Self {
        self.env = Some(env);
        self
    }
    ...
    fn create_resource(&self) -> Resource {
        ...
        let provided = Resource::new(vec![
            KeyValue::new(
                ...
                self.service_name.as_ref().map_or_else(
                    || std::env::var("FERMAH_SERVICE").unwrap_or_else(|_| "unknown".to_string()),
                    |name| name.to_string(),
                ),
            ),
            KeyValue::new(
            ...
            KeyValue::new(
                ...
                self.env.as_ref().map_or_else(
                    || std::env::var("ENV").unwrap_or_else(|_| "local".to_string()),
                    |name| name.to_string(),
                ),
            ),
        ]);

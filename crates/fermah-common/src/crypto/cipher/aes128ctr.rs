...
pub enum Aes128CtrCipherError {
    ...
    #[error("mac mismatch: {expected} != {found}")]
...
impl<KDF: Kdf> Cipher for Aes128CtrCipher<KDF> {
    ...
    fn encrypt(&mut self, password: &[u8]) -> Result<(), Self::Error> {
        ...
        hasher.update(&[cipher_key.as_slice(), self.data.as_slice()].concat());
...
mod tests {
    ...
    fn test_aes128ctr_cipher_encrypt() {
        ...
        assert_eq!(
            ...
            "0x586c5fcf802e6d0c02df2847c2129a0fdf808254b7635a4c41b36fc5a0baeaa0".to_string()
        );

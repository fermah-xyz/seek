...
impl KeyCommands {
        ...
        Error: From<<S as Signer>::SignerError>,
    {
        ...
        let address = signer.verifying_key_vec();
    ...
    async fn save_keys(
        ...
        let password = if pw_args.no_password {
            ...
            match &pw_args.password_stdin {
                true => KeystoreFile::get_password().await?,
        ...
        match password.is_empty() {
            false => {
                ...
                let keystore = KeystoreCipher::new(cipher, address.clone(), uuid);
            ...
            true => {
                let keystore =
                    KeystoreCipher::new(PlainCipher::new(private_key), address.clone(), uuid);
    ...
    fn prompt_password(pw_args: &PasswordArgs) -> Result<String, Error> {
        ...
        while password != confirm {
            ...
            confirm = cli::prompt_for_password_confirmation()?;
            if confirm != password {
                println!(

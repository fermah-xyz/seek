...
async fn run() -> Result<(), Error> {
    ...
    match cli.command {
        ...
        ClientCommands::Key { keys } => {
            t.with_directive(LevelFilter::WARN.into()).init();
        ...
        ClientCommands::Proof { proofs } => {
            ...
            match proofs {
                ProofCommands::SendProofRequest {
                    ...
                    let rpc = RpcClient::from_config(rpc, &key).await?;
        ...
        ClientCommands::Deposit {
            ...
            .await?;
            let client_contracts = Contracts::from_config(&avs, &chain_rpc, &key).await?;

            #[cfg(feature = "mint_vault_token")]
            {
                let minter_contracts =
                    Contracts::from_config(&avs, &chain_rpc, &KeystoreConfig { key: minter_key })
                        .await?;
                minter_contracts
                .fermah_contracts
                .vault_token
            ...
            RpcClient::from_config(RpcConfig { connection: mm_rpc }, &key)
                .await?
                .update_balance()
                .await?;
        ...
        ClientCommands::UpdateBalance { rpc, key } => {
            RpcClient::from_config(rpc, &key)
                .await?
                .update_balance()
                .await?;
        }
        ClientCommands::ReturnUnspent { rpc, key } => {
            RpcClient::from_config(rpc, &key)
                .await?
                .return_unspent()
                .await?;
